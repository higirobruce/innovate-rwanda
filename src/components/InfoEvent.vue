  ...
  <div v-if="post">
    ...
    <div class="px-4 py-2" style="max-height: 460px; overflow: auto">
      <div class="head-post">
        ...
        <div class="image" v-if="post.flyer">
          <img
    ...
    <div class="my-3 mx-4">
      <span class="float-left">
        ...
        <button
          ...
          @click="approveOrDecline('draft', post.id)"
...
export default {
  ...
  mounted() {
    AxiosHelper.get(`events/info/${this.id}`)
      .then((response) => {
  ...
  methods: {
    ...
    approveOrDecline(decision, id) {
      ...
      AxiosHelper.put("events/approve-decline", data)
        .then(() => {
          Vue.$toast.open({
            message:
              decision === "approved"
                ? "Event has been approved and published. We are updating list..."
